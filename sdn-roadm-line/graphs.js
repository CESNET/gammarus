...
const getData = async () => {
    ...
    if (window.location.href.match("logs")) {
        ...
        url = window.location.origin + "/+restconf/operations/czechlight-roadm-device:full-spectrum-scan"
    }
...
const main = async () => {
    ...
    let commonInData = innerData['common-in'].map((value) => { return {x: value.frequency, y: value.power}; }).sort((a, b) => a.x - b.x);
    let commonOutData = innerData['common-out'].map((value) => { return {x: value.frequency, y: value.power}; }).sort((a, b) => a.x - b.x);
