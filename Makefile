...
.PHONY: install install-binaries install-services install-sockets install-static-files install-nghttpx-device-specific
...
install-sockets: $(wildcard *.socket)
	mkdir -p $(DESTDIR)$(PREFIX)/lib/systemd/system/sockets.target.wants/
	cp $^ $(DESTDIR)$(PREFIX)/lib/systemd/system/
	$(foreach unit,$(call not-containing,@,$^),ln -sf ../$(unit) $(DESTDIR)$(PREFIX)/lib/systemd/system/sockets.target.wants/;)
...
install: install-binaries install-services install-sockets install-static-files install-nghttpx-device-specific
