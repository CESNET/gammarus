...
install-services: restconf-full-spectrum-scan.service nghttpx@.service nghttpd@.service
	mkdir -p $(DESTDIR)$(PREFIX)/lib/systemd/system/
	mkdir -p $(DESTDIR)$(PREFIX)/lib/systemd/system/multi-user.target.wants/
	cp $^ $(DESTDIR)$(PREFIX)/lib/systemd/system/
	$(foreach service,$(call not-containing,@,$^),ln -sf ../$(service) $(DESTDIR)$(PREFIX)/lib/systemd/system/multi-user.target.wants/;)
...
install-nghttpx-device-specific: $(wildcard nghttpx-conf/*.conf)
